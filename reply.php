...
$data = array($_POST['tweet'],$SESSION['id'], $_GET['tweet_id']);
...
// 返信する投稿の内容取得SQL文
// tweetsテーブルの全件、membersテーブルのnick_name, memebersテーブルのpicture_path
$sql = 'SELECT `tweets`.*, `nick_name`,`members`,`picture_path` FROM `tweets` LEFT JOIN `members` ON `tweets`.`member_id` = `members`.`member_id` WHERE `tweet_id`=?';
...
// レコード一件取得
$one_tweet = $stmt->fetch(PDO::FETCH_ASOOC);
